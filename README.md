...
![Vivado](https://img.shields.io/badge/Tool-AMD_Vivado-red.svg?style=for-the-badge)
...
│
├── Multicycle/
│   ├── RISC-V Multicycle.xpr
│   └── RISC-V Multicycle.srcs/
│       └── sources_1/
│           ├── bd/
│           └── new/
│
...
Each processor architecture is kept as an independent Vivado project. The `.xpr` files can be opened directly in Vivado, while the corresponding `.srcs/` directories contain the Block Design, HDL source files, testbenches, and IP configuration files required by each implementation.

Vivado-generated output directories such as `.cache`, `.gen`, `.hw`, `.ip_user_files`, `.sim`, and `.runs` are excluded from version control through `.gitignore`, since they can be regenerated by Vivado.
